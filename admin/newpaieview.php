<?php
    ...
    if(!empty($_POST)){
        ...
        try{
            ...
            $statement = $db->prepare("INSERT INTO paievalide (Guide, AmiLocal, Extension, SBN, CheckIn, PaieHT, PaieTTC, Extra, Depense, Commentaire, Date) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
            $statement->execute(array($guide,$amilocal,$extension,$sbn,$checkin,$paieht,$paiettc,$extra,$depense,$commentaire,$mois));

            $success = true;
            if($success){
                $statement = $db->prepare("DELETE FROM newpaie WHERE ID = ?");
    ...
    {
        $db = Database::connect();
        $statement = $db->prepare("SELECT * FROM newpaie WHERE ID = ?");
        $statement->execute(array($id));
    ...
    <div class="container-fluid">
        <div class="row">
            <section class="col-12 text-center">
                ...
                <form action="newpaieview.php" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="extension" name="extension" value="<?php echo $extension ?>"
                            readonly><br>
                    </div>
                    <div class="form-group">
