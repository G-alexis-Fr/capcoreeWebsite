<?php 
    ...
    if(!empty($_GET['id']))
        $idChambre = checkInput($_GET['id']); // On recupere l'ID de la chambre
    
    $typeError = $nbreLitError = $prixError = $type = $nbreLit = $prix = "";

    if(!empty($_POST)){
        $idChambre = checkInput($_POST['id']);
        ...
        if($isSuccess){
            ...
            $statement = $db->prepare("UPDATE chambre set IdHotel=?,TypeChambre=?,NbreLit=?,Prix=? WHERE ID=?");
            $statement->execute(array($hotel,$type,$nbreLit,$prix,$idChambre));
            Database::disconnect();
            header("Location: hotelsview.php?id=$hotel"); // Apres l'envoie on redirige a hotels.php
        };
    }else {
        // on recupere les donnees pour preremplir les textboxs 
        $db = Database::connect();
        $statement = $db->prepare("SELECT * FROM chambre WHERE ID = ?");
        $statement->execute(array($idChambre));
        $item = $statement->fetch();

        $hotel = $item["IdHotel"];
        $type = $item["TypeChambre"];
        $nbreLit = $item["NbreLit"];
        $prix = $item["Prix"];
...
<body>
    ...
    <div class="container-fluid text-center">
        <h1>Modifier Chambres</h1>
        <br><br>
        <div class="row text-center">

            <section class="col-12">
                <form action="<?php echo 'chambresupdate.php?id=' . $idChambre ?>" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select id="search" name="nbreLit" id="nbreLit">
                            <option value="<?php echo $nbreLit?>"><?php echo $nbreLit?></option>;
                    ...
                    <div class="form-group">
                        ...
                        <input id="search" type="text" id="prix" name="prix" placeholder="Prix *"
                            value="<?php echo $prix ?>"><br>
                        <span class="erreur"><?php echo $prixError; ?></span>
                    </div>

                    <div class="form-group">
                        <button class="btn btn-primary btn-lg" type="submit">MODIFIER</button>
                        <a class="btn btn-primary btn-lg" href="hotelsview.php?id=<?php echo $hotel ?>">RETOUR</a>
                    </div>
                </form>
            </section>
        </div>
