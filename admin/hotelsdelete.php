<?php
    ...
    if(!empty($_POST["id"])){ // Si l'utilisateur a clique sur OUI alors on supprime a partir de l'id
        ...
        $db = Database::connect();
        $statement = $db->prepare("DELETE FROM hotel WHERE ID = ?");
        $statement->execute(array($id));
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center">
                    <h1>Supprimer Hôtel</h1>
                    <br><br>
                    <form action="hotelsdelete.php" role="form" method="post">
            <input type="hidden" name="id" value="<?php echo $id;?>"/>
            <p >Etes vous sur de vouloir supprimer l'Hôtel <strong><?php echo " " . $item["Nom"]?>?</strong></p>
            <div class="form-group">
                <button class="btn btn-primary btn-lg" type="submit">SUPPRIMER</button>
                <a class="btn btn-primary btn-lg" href="hotels.php">RETOUR</a>
            </div>
        </form>
                </div>
            </div>
