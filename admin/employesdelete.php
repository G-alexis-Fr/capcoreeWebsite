<?php
    ...
    $db = Database::connect();
    $statement = $db->prepare("SELECT Role,Nom,Prenom FROM listeemploye WHERE ID = ?");
    ...
    if(!empty($_POST["id"])){ // Si l'utilisateur a clique sur OUI alors on supprime a partir de l'id
        ...
        $statement = $db->prepare("DELETE FROM listeemploye WHERE ID = ?");
        $statement->execute(array($id));

        Database::disconnect();
        header("Location: employes.php"); // Apres l'envoie on redirige a employes
    };
?>
    ...
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                ...
                <form action="employesdelete.php" role="form" method="post">
                    <input type="hidden" name="id" value="<?php echo $id;?>"/>
                    <p >Etes vous sur de vouloir supprimer <strong><?php echo $item["Role"] . " " . $item["Nom"] . " " . $item["Prenom"]?>?</strong></p>
